...
#define LCD_EN(x)   ((x) ? (GLCD_BL_PORT |= 1<<GLCD_BL_BIT) : (GLCD_BL_PORT &= ~(1<<GLCD_BL_BIT)));
/* Pin CS  0 o 1                                                   */
#define LCD_CS(x)   ((x) ? (GLCD_CS_PORT |= 1<<GLCD_CS_BIT) : (GLCD_CS_PORT &= ~(1<<GLCD_CS_BIT)));
/* Pin RS  0 o 1                                                   */
#define LCD_RS(x)   ((x) ? (GLCD_RS_PORT |= 1<<GLCD_RS_BIT) : (GLCD_RS_PORT &= ~(1<<GLCD_RS_BIT)));
/* Pin WR  0 o 1                                                   */
#define LCD_WR(x)   ((x) ? (GLCD_WR_PORT |= 1<<GLCD_WR_BIT) : (GLCD_WR_PORT &= ~(1<<GLCD_WR_BIT)));
/* Pin RD  0 o 1                                                   */
#define LCD_RD(x)   ((x) ? (GLCD_RD_PORT |= 1<<GLCD_RD_BIT) : (GLCD_RD_PORT &= ~(1<<GLCD_RD_BIT)));
...
static inline unsigned char lcd_send (unsigned short byte) {
	DATA_GLCD_DDR = 0xFF;
	DATA_GLCD_PORT = byte >> 8;
	LCD_WR(0)
	LCD_WR(1)
	DATA_GLCD_PORT = byte & 0xFF;
	LCD_WR(0)
	LCD_WR(1)
	return(1);
}
...
static inline unsigned short lcd_read (void) {
  unsigned short id;
	DATA_GLCD_DDR = 0x00;
	LCD_RD(0)
	id=DATA_GLCD_PIN &0x00FF;
	LCD_RD(1)
	LCD_RD(0)
	id=(id << 8) | (DATA_GLCD_PIN &0x00FF);
	LCD_RD(1)
  	return(id); 
}
